...
BTnode* mergeBST(BTnode *bt1, BTnode *bt2){
	...
	if (bt1==NULL && bt2==NULL){
		...
		x = new BTnode;
		x->data = bt1->data;
		x->right = mergeBST(bt1->right, NULL);
		x->left = mergeBST(bt1->left, NULL);
        }
    else if (bt2 != NULL && bt1 ==NULL){
		x = new BTnode;
		x->data = bt2->data;
		x->left = mergeBST(NULL, bt2->left);
		x->right = mergeBST(NULL, bt2->right);
        }
    else {
		x = new BTnode;
		x->data = bt1->data + bt2->data;
		x->left = mergeBST(bt1->left, bt2->left);
		x->right = mergeBST(bt1->right, bt2->right);
        }
	return x;
}
